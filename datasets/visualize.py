    ...
    def _visualize(self, iq_data: np.ndarray, targets: np.ndarray) -> Figure:
        ...
        for sample_idx in range(batch_size):
            ...
            _, _, spectrogram = sp.spectrogram(
                x=iq_data[sample_idx][:, 0] + 1j * iq_data[sample_idx][:, 1],
                fs=self.sample_rate,
                window=self.window,
    ...
    def _visualize(self, iq_data: np.ndarray, targets: np.ndarray) -> Figure:
        ...
        for sample_idx in range(batch_size):
            ...
            cwt_matrix, freqs = pywt.cwt(
                iq_data[sample_idx][:, 0] + 1j * iq_data[sample_idx][:, 1],
                scales=scales,
...
class ConstellationVisualizer(Visualizer):
    ...
    def _visualize(self, iq_data: np.ndarray, targets: np.ndarray) -> Figure:
        ...
        for sample_idx in range(batch_size):
            plt.subplot(int(np.ceil(np.sqrt(batch_size))),
                        int(np.sqrt(batch_size)), sample_idx + 1)
            plt.scatter(iq_data[sample_idx][:, 0],
                        iq_data[sample_idx][:, 1])
            plt.xticks([])
            plt.yticks([])
            plt.title(str(targets[sample_idx]))
        return figure
...
class IQVisualizer(Visualizer):
    ...
    def _visualize(self, iq_data: np.ndarray, targets: np.ndarray) -> Figure:
        ...
        for sample_idx in range(batch_size):
            plt.subplot(int(np.ceil(np.sqrt(batch_size))),
                        int(np.sqrt(batch_size)), sample_idx + 1)
            plt.plot(iq_data[sample_idx][:, 0])
            plt.plot(iq_data[sample_idx][:, 1])
            plt.xticks([])
            plt.yticks([])
            plt.title(str(targets[sample_idx]))
        return figure
    ...
    def _visualize(self, iq_data: np.ndarray, targets: np.ndarray) -> Figure:
        ...
        for sample_idx in range(batch_size):
            plt.subplot(int(np.ceil(np.sqrt(batch_size))),
                        int(np.sqrt(batch_size)), sample_idx + 1)
            x = iq_data[sample_idx][:, 0] + 1j * iq_data[sample_idx][:, 1]
            Pxx, freqs = plt.psd(x, NFFT=self.fft_size, Fs=1)
            plt.xticks()
            plt.yticks()
            plt.title(str(targets[sample_idx]))
        return figure
